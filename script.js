...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let calendarEventsData = {}; // **新增**: 存放從後端獲取的日曆事件
    ...
    // **月曆頁面相關元素**
    const calendarPageContainer = document.querySelector('#page-calendar .container'); // **修改**: 獲取整個容器
    ...
    // ================== 核心：頁面切換邏輯 ==================
    async function switchPage(targetId) { // **修改**: 改為 async function
        ...
        if (targetId === 'page-calendar') await fetchCalendarEvents(); // **修改**: 改為獲取遠端資料
    }
    ...
    if (registerBtn) registerBtn.addEventListener('click', async () => { /* ...無變動... */ });
    if (loginBtn) loginBtn.addEventListener('click', async () => { /* ...無變動... */ });
    if (logoutBtn) logoutBtn.addEventListener('click', () => { Auth.clearToken(); stopChatSession(); switchPage('page-home'); alert('您已成功登出。'); }); // **修改**: 增加提示
    ...
    async function fetchChatMessages() { /* ...無變動... */ }
    async function sendChatMessage() { /* ...無變動... */ }
    // (無變動部分省略)
    ...
    async function fetchCalendarEvents() {
        ...
        try {
            ...
            renderCalendar(); // 初始渲染日曆
            // 預設選中今天
            const today = new Date();
            const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
            renderEventDetails(todayStr); // 預設顯示今天的詳情
    ...
    function renderCalendar(selectedDate) {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            if (calendarEventsData[dateStr]) dayElement.classList.add('has-event');
            if (dateStr === selectedDate) dayElement.classList.add('today'); // "today" class now means "selected"
    ...
    function renderEventDetails(dateStr) {
        ...
        renderCalendar(dateStr); // 重新渲染日曆以更新選中樣式

        const event = calendarEventsData[dateStr];
        ...
        if (!event) {
            ...
            if (!Auth.isLoggedIn()) {
                ...
                orderSectionHtml = `<p class="order-status">本日點餐已截止。</p>`;
            } else {
                ...
                orderSectionHtml = `
                    <div class="order-submission-area">
                        ...
                        <textarea id="order-textarea" class="order-textarea" placeholder="範例：招牌鍋貼一份，豆漿一杯">${orderText}</textarea>
                        <button id="order-submit-btn" class="auth-btn">送出/更新訂單</button>
                    </div>
                `;
            }
        ...
        // 動態綁定事件
        const loginLink = document.getElementById('login-from-calendar');
        if(loginLink) loginLink.addEventListener('click', (e) => { e.preventDefault(); switchPage('page-login'); });

        const submitBtn = document.getElementById('order-submit-btn');
        if(submitBtn) submitBtn.addEventListener('click', () => handleOrderSubmit(dateStr));
    }
    
    async function handleOrderSubmit(dateStr) {
        const token = Auth.getToken();
        if(!token) return alert("請先登入！");

        const textarea = document.getElementById('order-textarea');
        const orderText = textarea.value; // 不 trim，允許只輸入空格
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/calendar/order`, {
                ...
                body: JSON.stringify({ date: dateStr, orderText: orderText })
            });
    ...
    // ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            ...
            { 
                name: "39焗烤家族", 
                phone: "077778883", 
                address: "高雄市鳳山區八德路二段260號", 
                info: "電話: 07-7778883\n週二公休\n此為連鎖店通用菜單，需確認訂購分店。\n與鳳山高中距離：約 2.5 公里 (開車約 7 分鐘，步行約 30 分鐘)", 
                tags: ["焗烤", "closed:週二公休"], 
                menu: [
                    { type: 'subtitle', text: '升級套餐說明:' },
                    { name: 'No.1', price: '單點' },
                    { name: 'No.2 (+飲料)', price: '主餐價+30元' },
                    { name: 'No.3 (+濃湯)', price: '主餐價+30元' },
                    { name: 'No.4 (+飲料+濃湯)', price: '主餐價+50元' },
                    { type: 'subtitle', text: '比薩 (Pizza) (皆為6吋)' },
                    { name: '玉米/夏威夷/蔬菜總匯/培根/燻雞/泡菜/鮪魚/鮮蝦/辣味雞丁/德式香腸', price: '單點 99元 / No.2 149元 / No.3 159元 / No.4 169元' },
                    { name: '章魚燒/燻雞蘑菇/羅勒燻雞/泡菜牛肉/泡菜豬肉/總匯/煙燻德腸', price: '單點 109元 / No.2 159元 / No.3 169元 / No.4 179元' },
                    { name: '龍蝦沙拉', price: '單點 119元 / No.2 179元 / No.3 189元 / No.4 199元' },
                    { name: '起司加倍', price: '+35元' },
                    { type: 'subtitle', text: '義大利麵 (Spaghetti)' },
                    { name: '蔬菜/奶油/茄汁/黑胡椒/肉醬', price: '單點 79元 / No.2 139元 / No.3 149元 / No.4 159元' },
                    { name: '羅勒/奶油燻雞/辣味雞/泡菜/韓國辣雞/奶油培根/海鮮/牛肉', price: '單點 99元 / No.2 159元 / No.3 169元 / No.4 179元' },
                    { name: '白醬焗烤', price: '+15元' },
                    { type: 'subtitle', text: '焗烤 (Pasta Baked) (飯/麵)' },
                    { name: '夏威夷/蔬菜/肉醬/培根/燻雞/泡菜/海鮮/鮪魚/牛肉/辣味雞丁/墨西哥/蕈菇燻雞/章魚燒/羅勒燻雞/奶油培根/咖哩雞肉/奶油雞肉/泡菜豬肉/泡菜牛肉/總匯/煙燻德腸', price: '單點 109元 / No.2 169元 / No.3 179元 / No.4 189元' },
                    { name: '龍蝦沙拉/法式燻鴨/檸檬雞柳', price: '單點 119元 / No.2 179元 / No.3 189元 / No.4 199元' },
                    { name: '起司加倍', price: '+25元' },
                    { name: '白醬焗烤', price: '+15元' },
                    { type: 'subtitle', text: '漢堡/點心 (Hamburger/Dessert)' },
                    { name: '薯條', price: 45 },
                    { name: '卡啦雞腿堡 (原味/辣味)', price: '單點 55元 / 套餐 115元' },
                    { name: '醬燒烤肉堡', price: '單點 65元 / 套餐 135元' },
                    { name: '熱狗', price: 29 },
                    { name: '麥克雞塊', price: 29 },
                    { name: '辣味雞米花', price: 65 },
                    { name: '檸檬雞柳條', price: 65 },
                    { type: 'subtitle', text: '特製排餐 (Special)' },
                    { name: '紐奧良雞排/卡啦雞/醬燒豬排/茄汁豬排/泡菜豬排/泡菜雞排', price: 145 },
                    { type: 'subtitle', text: '素食 (Vegetarian) (飯/麵)' },
                    { name: '夏威夷/蔬菜', price: '單點 99元' },
                    { name: '素肉燥', price: 89 },
                    { name: '素什錦', price: 119 },
                    { name: '素食總匯', price: 109 },
                    { name: '素什錦(鐵板)', price: 109 },
                    { name: '無肉玉米', price: 99 },
                    { name: '素玉米什錦', price: 119 },
                    { name: '無肉玉米(鐵板)', price: 65 },
                    { name: '玉米濃湯', price: 40 },
                    { name: '蔬菜玉米濃湯', price: 65 },
                    { type: 'subtitle', text: '冷飲 (Drinks)' },
                    { name: '紅茶/綠茶/百香果/柳橙汁/可樂/雪碧/蔓越莓/錫蘭奶茶/梅子紅', price: 30 },
                    { name: '柳橙汁/鮮奶茶', price: 40 },
                    { name: '冬瓜檸檬/冬瓜鮮奶/阿華田/鮮奶綠', price: 40 },
                    { name: '加料: 奶蓋/珍珠/椰果', price: '+10' }
                ]
            },
            { 
                name: "昭和炒飯", 
                phone: "077228585", 
                address: "高雄市苓雅區廣州一街141-5號", 
                info: "電話: 07-7228585\n**注意：店家位於苓雅區，與鳳山高中距離約 4.0 公里 (開車約 10 分鐘)，請務必確認外送範圍。**\n此為連鎖店通用菜單，需確認訂購分店", 
                tags: ["炒飯"], 
                menu: [
                    { type: 'subtitle', text: '招牌系列' },
                    { name: '昭和牛丼', price: 160 },
                    { name: '昭和炒飯', price: 145 },
                    { name: '海鮮什錦炒飯', price: 140 },
                    { name: '泡菜牛肉蓋飯', price: 170 },
                    { name: '揚州炒飯', price: 140 },
                    { name: '咖哩海陸蓋炒飯', price: 140 },
                    { name: '日式芥末鮭魚炒飯', price: 115 },
                    { type: 'subtitle', text: '家常炒飯' },
                    { name: '蝦仁蛋炒飯', price: 100 },
                    { name: '黃金鮭魚蛋炒飯', price: 110 },
                    { name: '泰式香雞蛋炒飯', price: 100 },
                    { name: '櫻花蝦蛋炒飯', price: 95 },
                    { name: '肉絲蛋炒飯', price: 90 },
                    { name: '雞丁蛋炒飯', price: 90 },
                    { name: '香腸蛋炒飯', price: 90 },
                    { name: '培根蛋炒飯', price: 80 },
                    { name: '火腿蛋炒飯', price: 75 },
                    { name: '蛋炒飯', price: 70 },
                    { name: '蔬菜玉米蛋炒飯', price: 80 },
                    { name: '青椒肉絲蛋炒飯', price: 100 },
                    { name: '青椒牛肉蛋炒飯', price: 110 },
                    { type: 'subtitle', text: '熱炒系列' },
                    { name: '現炒青菜', price: 65 },
                    { name: '香煎蔥蛋', price: 65 },
                    { name: '香煎蝦仁蛋', price: 80 },
                    { name: '香煎菜圃蛋', price: 70 },
                    { name: '培根高麗菜', price: 75 },
                    { type: 'subtitle', text: '風味炒麵 (加麵+15元)' },
                    { name: '海鮮什錦炒烏龍', price: 135 },
                    { name: '沙茶牛肉炒麵', price: 105 },
                    { name: '沙茶豬肉炒麵', price: 100 },
                    { name: '沙茶羊肉炒麵', price: 105 },
                    { name: '金門炒泡麵', price: 95 },
                    { name: '炒泡麵', price: 90 },
                    { name: '泡菜炒烏龍', price: 110 },
                    { name: '咖哩牛肉炒烏龍', price: 115 },
                    { name: '咖哩豬肉炒烏龍', price: 100 },
                    { name: '日式炒烏龍麵', price: 95 },
                    { name: '台式炒麵', price: 95 },
                    { type: 'subtitle', text: '風味炒飯' },
                    { name: '醬燒牛肉蛋炒飯', price: 125 },
                    { name: '醬燒肉絲蛋炒飯', price: 115 },
                    { name: '醬燒雞丁蛋炒飯', price: 115 },
                    { name: '沙茶羊肉蛋炒飯', price: 110 },
                    { name: '沙茶牛肉蛋炒飯', price: 110 },
                    { name: '沙茶豬肉蛋炒飯', price: 95 },
                    { name: '咖哩牛肉蛋炒飯', price: 120 },
                    { name: '咖哩雞肉蛋炒飯', price: 100 },
                    { name: '咖哩豬肉蛋炒飯', price: 100 },
                    { name: '泡菜炒飯', price: 120 },
                    { name: '番茄牛肉蛋炒飯', price: 120 },
                    { name: '番茄肉絲蛋炒飯', price: 100 },
                    { name: '番茄蛋炒飯', price: 90 },
                    { name: '夏威夷鳳梨炒飯', price: 105 },
                    { name: '蒜香奶油培根炒飯', price: 95 },
                    { name: '蒜香奶油鮭魚炒飯', price: 110 },
                    { name: '麻辣炒飯', price: 120 },
                    { name: '麻辣牛肉炒飯', price: 130 },
                    { type: 'subtitle', text: '焗烤系列' },
                    { name: '海鮮什錦焗飯', price: 135 },
                    { name: '咖哩牛肉焗飯', price: 115 },
                    { name: '咖哩雞肉焗飯', price: 100 },
                    { name: '沙茶牛肉焗飯', price: 110 },
                    { name: '沙茶羊肉焗飯', price: 110 },
                    { name: '沙茶豬肉焗飯', price: 95 },
                    { name: '日式醬燒牛肉焗飯', price: 125 },
                    { name: '日式醬燒豬肉焗飯', price: 115 },
                    { type: 'subtitle', text: '鍋燒系列 (可選：意麵、烏龍、冬粉、泡麵)' },
                    { name: '原味鍋燒', price: 95 },
                    { name: '奶油鍋燒', price: 130 },
                    { name: '麻辣鍋燒', price: 120 },
                    { type: 'subtitle', text: '湯品 & 其他' },
                    { name: '蛋花湯', price: 40 },
                    { name: '紫菜蛋花湯', price: 45 },
                    { name: '貢丸湯', price: 45 },
                    { name: '魚丸湯', price: 45 },
                    { name: '蛤仔湯', price: 55 },
                    { name: '泡菜(單點)', price: 50 },
                    { name: '泡菜(罐裝)', price: 230 },
                    { name: '加起司', price: 30 },
                    { name: '加蛋', price: 15 },
                    { type: 'subtitle', text: '飲品' },
                    { name: '梨山青茶', price: 50 },
                    { name: '冬瓜檸檬', price: 55 },
                    { name: '黑糖沖繩鮮奶茶', price: 55 }
                ]
            },
            { 
                name: "何媽媽鍋燒丼飯炒飯", 
                phone: "077263740", 
                address: "高雄市苓雅區福德三路269號", 
                info: "電話: 07-7263740 / 0956-536299\n營業時間: 上午9:00~13:00 / 下午4:00~7:30\n**注意：店家位於苓雅區，與鳳山高中距離約 2.8 公里 (開車約 7 分鐘，步行約 34 分鐘)，請務必確認外送範圍。**", 
                tags: ["鍋燒", "丼飯", "炒飯", "自製滷油"], 
                menu: [
                    { type: 'subtitle', text: '限量咖哩系列' },
                    { name: '豬排咖哩飯', price: 100 },
                    { name: '雞排咖哩飯', price: 100 },
                    { name: '魚排咖哩飯', price: 100 },
                    { name: '牛肉咖哩飯', price: 100 },
                    { type: 'subtitle', text: '丼飯系列' },
                    { name: '豬肉丼飯', price: 100 },
                    { name: '雞肉丼飯', price: 100 },
                    { name: '豬排丼飯', price: 100 },
                    { name: '雞排丼飯', price: 100 },
                    { name: '魚排丼飯', price: 100 },
                    { name: '牛肉丼飯', price: 100 },
                    { type: 'subtitle', text: '人氣炒飯系列 (加滷油+10)' },
                    { name: '火腿炒飯', price: 80 },
                    { name: '培根炒飯', price: 80 },
                    { name: '肉絲炒飯', price: 80 },
                    { name: '蝦仁炒飯', price: 90 },
                    { name: '蚵仁炒飯', price: 90 },
                    { name: '滷油雞粒炒飯', price: 90 },
                    { name: '青椒肉絲炒飯', price: 90 },
                    { name: '綜合炒飯', price: 90 },
                    { name: '素食蛋炒飯', price: 90 },
                    { name: '香腸蛋炒飯', price: 90 },
                    { name: '青椒肉絲炒飯', price: 90 },
                    ...
                    { name: '鹹豬肉炒飯', price: 90 },
                    { name: '豬肉炒飯', price: 90 },
                    { type: 'subtitle', text: '湯系列' },
                    { name: '蛋花湯', price: 35 },
                    { name: '貢丸湯', price: 35 },
                    { name: '蛤仔湯', price: 40 },
                    { name: '魚皮湯', price: 40 },
                    { name: '滷油雞腿湯', price: 85 },
                    { type: 'subtitle', text: '鍋燒系列 (滷油加10)' },
                    { name: '麻婆豆腐燴飯', price: 100 },
                    { name: '滷油雞腿燴飯', price: 100 },
                    { name: '蝦仁燴飯', price: 100 },
                    { name: '沙茶雞肉/豬肉燴飯', price: 100 },
                    { name: '鍋燒系列', price: 85 },
                    { name: '鍋燒雞絲', price: 85 },
                    { name: '鍋燒烏龍', price: 85 },
                    { name: '鍋燒冬粉', price: 85 },
                    ...
                    { name: '鍋燒拉麵', price: 85 },
                    { name: '鍋燒泡麵', price: 85 },
                    { type: 'subtitle', text: '炒麵系列 (滷油加10)' },
                    { name: '炒意麵', price: 85 },
                    { name: '炒烏龍', price: 85 },
                    { name: '炒泡麵', price: 85 },
                    { type: 'subtitle', text: '其他系列' },
                    { name: '炒青菜', price: 60 },
                    { name: '煎鴨蛋', price: 60 },
                    { name: '魚皮粥', price: 70 },
                    { name: '魚肉粥', price: 70 },
                    { name: '水餃 (最少十粒)', price: '6元/粒' },
                    { name: '炸豬排', price: 60 },
                    { name: '炸雞排', price: 60 }
                ]
            },
            { 
                name: "吳爸便當", 
                phone: "077775102", 
                address: "高雄市鳳山區文衡路189號", 
                info: "營業時間: 09:00 - 20:00\n公休日: 每週六公休\n電話: 07-7775102 / 0968-788-909\n與鳳山高中距離: 約 1.8 公里 (開車約 5 分鐘，步行約 22 分鐘)", 
                tags: ["便當", "closed:週六公休"], 
                menu: [
                    { type: 'subtitle', text: '人氣主餐' },
                    { name: '台灣三寶飯 (香腸+鹹豬肉+無骨腿)', price: 90 },
                    { name: '韓國炸雞', price: 90 },
                    { name: '招牌油雞', price: 90 },
                    { name: '辣妹椒麻雞', price: 110 },
                    { type: 'subtitle', text: '便當' },
                    { name: '招牌雙拼飯 (油雞+炸排)', price: 130 },
                    { name: '炸雞排便當', price: 110 },
                    { name: '辦桌炸雞腿飯', price: 90 },
                    { name: '無骨雞腿排飯', price: 90 },
                    { name: '泰式椒麻魚', price: 80 },
                    { name: '炸排骨飯', price: 80 },
                    { name: '宮保雞丁飯', price: 80 },
                    { name: '招牌滷排骨飯', price: 80 },
                    { name: '鹹蛋瓜仔肉飯', price: 80 },
                    { name: '糖醋咕咾肉飯', price: 80 },
                    { name: '香煎雞腿排飯', price: 80 },
                    { name: '台味滷雞腿飯', price: 80 },
                    { name: '鯖魚飯', price: 80 },
                    { name: '鱈魚飯', price: 80 },
                    { name: '香腸便當', price: 75 },
                    { name: '鹹豬雞便當', price: 75 },
                    { name: '鹹豬肉便當', price: 75 },
                    { name: '古早味肉燥飯', price: 70 },
                    { type: 'subtitle', text: '綜合便當' },
                    { name: '炸雞腿 + 宮保雞丁', price: 90 },
                    { name: '炸雞腿 + 鯖魚', price: 90 },
                    { name: '炸雞腿 + 咕咾肉', price: 90 },
                    { type: 'subtitle', text: '菜飯便當' },
                    { name: '四菜', price: 75 },
                    { name: '三菜', price: 65 },
                    { name: '二菜', price: 55 }
                ]
            ...
            { 
                name: "老闆不夠辣", 
                phone: "077998855", 
                address: "高雄市鳳山區光復路一段1號", 
                info: "營業時間: 11:00-13:50 / 16:00-20:00\n與鳳山高中距離: 約 0.6 公里 (開車約 2 分鐘，步行約 7 分鐘)\n備註: 餐點可自選加價升級為丼飯、咖哩飯、焗烤", 
                tags: ["麵食", "小菜", "咖哩"], 
                menu: [
                    { type: 'subtitle', text: '招牌咖哩飯' },
                    { name: '單點咖哩飯', price: 49 },
                    { name: '快樂兒童咖哩飯', price: 65 },
                    { name: '可樂餅咖哩飯', price: 80 },
                    { name: '鹽酥杏鮑菇咖哩飯', price: 90 },
                    { name: '卡拉雞咖哩飯', price: 90 },
                    { name: '炸豬排咖哩飯', price: 90 },
                    { name: '起司豬排咖哩飯', price: 100 },
                    { name: '黃金雞腿咖哩飯', price: 120 },
                    { name: '酥炸大雞排咖哩飯', price: 120 },
                    { name: '唐揚雞咖哩飯', price: 130 },
                    { name: '厚切豬排咖哩飯', price: 135 },
                    { name: '雜豬雙享咖哩飯', price: 130 },
                    { name: '海陸空霸王咖哩飯', price: 188 },
                    { type: 'subtitle', text: '嫩肉咖哩飯' },
                    { name: '國產嫩豬咖哩飯', price: 120 },
                    { name: '舒肥嫩雞咖哩飯', price: 120 },
                    { name: '紅燒燉牛咖哩飯', price: 140 },
                    { type: 'subtitle', text: '老闆推薦小點' },
                    { name: '煉乳銀絲卷', price: 45 },
                    { name: '鹽酥杏鮑菇', price: 45 },
                    { name: '和風胡麻豆腐', price: 45 },
                    { name: '甜不辣', price: 45 },
                    { name: '起司地瓜條', price: 45 },
                    { name: '紫金雙色QQ球', price: 45 },
                    { name: '洋蔥圈', price: 45 },
                    { name: '麥克雞塊', price: 45 },
                    { name: '可樂餅', price: 45 },
                    { name: '港式心菜', price: 25 },
            ...
            { 
                name: "華新牛排", 
                phone: "077433522", 
                address: "高雄市鳳山區瑞興路260號", 
                info: "電話: 07-7433522 / 07-7431239 / 0930-035017\n與鳳山高中距離: 約 2.5 公里 (開車約 7 分鐘，步行約 30 分鐘)\n備註: 滿500元外送到府，隨餐附濃湯、飲料及餐包", 
                tags: ["牛排"], 
                menu: [
                    { type: 'subtitle', text: '排餐' },
                    { name: '超大厚切牛排', price: 180 },
                    { name: '龍力牛排', price: 150 },
                    { name: '頂級沙朗', price: 150 },
                    { name: '海陸大餐', price: 150 },
                    { name: '厚重牛排', price: 130 },
                    { name: '羊小排', price: 130 },
                    { name: '雙拼排', price: 130 },
                    { name: '大比目魚排', price: 120 },
                    { name: '胡椒鹹鮮蝦排', price: 120 },
                    { name: '香辣厚重豬排', price: 120 },
                    { name: '厚重豬排', price: 110 },
                    { name: '綜合排', price: 100 },
                    { name: '羊排', price: 100 },
                    { name: '雞腿排', price: 90 },
            ...
            { 
                name: "奇奇蚵仔煎", 
                phone: "077905029", 
                address: "高雄市鳳山區後庄街78號", 
                info: "營業時間:\n週一至週五: 11:00-14:45 / 16:00-21:00\n週六至週日: 11:00-21:00\n與鳳山高中距離: 約 3.8 公里 (開車約 10 分鐘，步行約 45 分鐘)", 
                tags: ["蚵仔煎"], 
                menu: [
                    { type: 'subtitle', text: '大咻 (蚵仔煎系列)' },
                    { name: '大咻鮮蚵 (原味/蒜味/胡麻)', price: '77 / 87 / 87' },
                    { name: '大咻蝦仁 (原味/蒜味/胡麻)', price: '87 / 97 / 97' },
                    { name: '大咻花枝 (原味/蒜味/胡麻)', price: '87 / 97 / 97' },
                    { name: '大咻干貝 (原味/蒜味/胡麻)', price: '107 / 117 / 117' },
                    { type: 'subtitle', text: '可口 (酥脆系列)' },
                    { name: '可口蚵仔燒', price: 97 },
                    { name: '可口蝦仁燒', price: 117 },
                    { name: '可口花枝燒', price: 117 },
                    { name: '可口干貝燒', price: 137 },
                    { name: '可口沙拉燒', price: 147 },
                    { type: 'subtitle', text: '奇饗 (主食)' },
                    { name: '奇饗鮮蚵煎', price: 97 },
                    { name: '奇饗蝦仁煎', price: 117 },
                    { name: '奇饗花枝煎', price: 117 },
                    { name: '奇饗干貝煎', price: 137 },
                    { name: '奇饗綜合煎', price: 147 },
                    { type: 'subtitle', text: '配料品 (加料)' },
                    { name: '雞蛋/青菜', price: '+17' },
                    { name: '麵/粿/飯/冬粉', price: '+17' },
                    { name: '鮮蚵/蝦仁/花枝/干貝', price: '+67' },
                    { name: '起司/蝦卵/蝦仁/鯛魚', price: '+37' },
                    { name: '沙茶醬(乾)/麻辣醬(乾)', price: '+7' },
                    { type: 'subtitle', text: '配料品 (單點)' },
                    { name: '大咻蛤蠣湯', price: 37 },
                    { name: '大咻蚵仔蛋', price: 57 },
                    { name: '大咻蝦仁蛋', price: 57 },
                    { type: 'subtitle', text: '飲品' },
            ...
            { 
                ...
                displayName: "李嘉義火雞肉飯", 
                phone: "077474905", 
                address: "高雄市鳳山區青年路二段140號", 
                info: "電話: 747-4905 / 專線 0928-860568\n營業時間: 09:30 - 19:30\n公休日: 每星期日 (若有第五個星期日則正常營業)\n與鳳山高中距離: 約 2.1 公里 (開車約 6 分鐘，步行約 26 分鐘)", 
                tags: ["飯", "closed:週日公休"], 
                menu: [
                    { type: 'subtitle', text: '便當系列 (附3樣菜)' },
                    { name: '火雞肉飯特製便當', price: 85 },
                    { name: '肉燥飯特製便當', price: 85 },
                    { name: '吳郭魚特製便當', price: 90 },
                    { type: 'subtitle', text: '飯類' },
                    { name: '火雞肉飯 (內用/外帶)', price: '小45 / 大55' },
                    { name: '火雞肉飯 (外帶)', price: '小45 / 大70 (外帶大碗份量較多)' },
                    { name: '肉燥飯 (內用/外帶)', price: '小40 / 大50' },
                    { name: '肉燥飯 (外帶)', price: '小40 / 大60 (外帶大碗份量較多)' },
                    { type: 'subtitle', text: '切盤系列' },
                    { name: '火雞肉切盤', price: 80 },
                    { name: '火雞翅切盤', price: 100 },
                    { name: '火雞元寶切盤', price: 100 },
                    { type: 'subtitle', text: '湯類' },
                    { name: '薑絲過魚湯', price: 130 },
                    { name: '虱目魚皮湯', price: 55 },
                    { name: '火雞下水湯', price: 35 },
                    { name: '火雞腳湯', price: 35 },
                    { name: '脆筍湯', price: 35 },
            ...
            { 
                name: "山內村", 
                displayName: "山內村咖哩 (鳳民店)", 
                phone: "073963077", 
                address: "高雄市三民區山東街117號", 
                info: "電話: (07) 396-3077 / 093317890\n營業時間: 11:00-14:00 / 17:00-20:00\n公休日: 週二公休\n**注意：店家位於三民區，與鳳山高中距離約 4.9 公里 (開車約 12 分鐘，步行約 1 小時)，請務必確認外送範圍。**\n備註:\n升級太陽蛋 +15元\n升級歐姆蛋 +30元\n青醬加倍 +30元", 
                tags: ["咖哩", "closed:週二公休"], 
                menu: [
                    { type: 'subtitle', text: '熟成濃咖哩' },
                    { name: '原味熟成咖哩飯 (無肉)', price: 70 },
                    { name: '豬肉熟成咖哩飯', price: 90 },
                    { name: '實格斯牛五花咖哩飯', price: 110 },
                    { name: '舒肥香料雞肉咖哩飯', price: 100 },
                    { name: '德式香腸熟成咖哩飯', price: 100 },
                    { type: 'subtitle', text: '麻婆豆腐咖哩' },
                    { name: '麻婆豆腐咖哩飯', price: 80 },
                    { name: '豬肉麻婆豆腐咖哩飯', price: 100 },
                    { name: '牛五花麻婆豆腐咖哩飯', price: 120 },
                    { name: '舒肥香料雞肉麻婆豆腐咖哩飯', price: 110 },
                    { name: '德式香腸麻婆豆腐咖哩飯', price: 110 },
                    { type: 'subtitle', text: '單點' },
                    { name: '白飯', price: 15 },
                    { name: '咖哩醬(一碗)', price: 40 },
                    { name: '胡麻小黃瓜', price: 40 },
                    { name: '胡麻花椰菜', price: 40 },
                    { name: '日式牛蒡絲', price: 40 },
                    { name: '涼拌海帶絲', price: 40 },
                    { name: '鮮蝦貝柱味噌湯', price: 40 },
                    { name: '甘露煮杏鮑菇', price: 40 },
                    { name: '和風芥末海帶芽', price: 40 },
                    { name: '黃金泡菜', price: 40 }
                ]
            },
            ...
            { 
                name: "鳳城老牌米糕", 
                displayName: "鳳城老牌米糕肉焿", 
                phone: "077418180", 
                address: "高雄市鳳山區維新路20號", 
                info: "電話: 07-741-8180\n與鳳山高中距離: 約 1.1 公里 (開車約 4 分鐘，步行約 14 分鐘)", 
                tags: ["米糕", "肉焿"], 
                menu: [
                    { type: 'subtitle', text: '主食' },
                    { name: '米糕', price: 40 },
                    { name: '肉焿', price: 45 },
                    { name: '飯焿', price: 55 },
                    { name: '麵焿', price: 55 },
                    { name: '米粉焿', price: 55 },
                    { name: '冬粉焿', price: 55 },
                    { type: 'subtitle', text: '湯品' },
                    { name: '貢丸湯', price: 30 },
                    { name: '魚丸湯', price: 30 },
                    { name: '貢丸冬粉湯', price: 40 },
                    { name: '魚丸冬粉湯', price: 40 },
                    { type: 'subtitle', text: '小菜' },
                    { name: '油豆腐', price: 10 },
                    { name: '滷蛋', price: '10 (兩顆15元)' },
                    { name: '滷貢丸', price: 8 }
                ]
            },
            ...
            { 
                name: "一翻馫", 
                phone: "0976667183", 
                address: "高雄市鳳山區自由路28-2號", 
                info: "訂購專線: 0976-667-183\n營業時間: 每週三固定公休，其餘時間不定時公休，請見店家LINE公告\n與鳳山高中距離: 約 1.4 公里 (開車約 4 分鐘，步行約 17 分鐘)\n備註:\n一公里內滿250元可外送\n麻辣湯底含牛油\n不吃蔥/蒜/洋蔥/胡蘿蔔/海鮮等，請提前告知", 
                tags: ["飯糰", "closed:週三公休"], 
                menu: [
                    { type: 'subtitle', text: '鍋燒 (可選：意麵/雞絲麵/冬粉/泡麵，換烏龍 +15元)' },
                    { name: '原味', price: 85 },
                    { name: '石頭', price: 90 },
                    { name: '沙茶', price: 90 },
                    { name: '泡菜', price: 100 },
                    ...
                    { name: '麻辣(豬)', price: 110 },
                    { name: '麻奶(豬)', price: 120 },
                    { name: '藤椒酸湯(豬)', price: 130 },
                    { type: 'subtitle', text: '蓋飯' },
                    { name: '回鍋肉蓋飯', price: 100 },
                    { name: '醬燒五花蓋飯', price: 100 },
                    { name: '日式培根牛丼飯', price: 110 },
                    { name: '醬燒松阪豬蓋飯', price: 110 },
                    { name: '蔥爆牛肉蓋飯', price: 110 },
                    { name: '沙茶牛肉蓋飯', price: 110 },
                    { name: '醬燒牛胸腹蓋飯', price: 110 },
                    { name: '香蔥油雞蓋飯', price: 120 },
                    { name: '照燒雞腿蓋飯', price: 120 },
                    { name: '紐奧良雞腿蓋飯', price: 120 },
                    { name: '炸豬排蓋飯', price: 120 },
                    { name: '柚香嫩肩牛蓋飯', price: 140 },
                    { type: 'subtitle', text: '炒飯/麵' },
                    { name: '火腿炒飯', price: 80 },
                    { name: '培根炒飯', price: 85 },
                    { name: '肉絲炒飯', price: 85 },
                    { name: '香腸炒飯', price: 90 },
                    ...
                    { name: '牛肉炒飯', price: 100 },
                    { name: '什錦炒飯', price: 100 },
                    { name: '老乾媽珠貝炒飯', price: 120 },
                    { name: '什錦炒麵', price: 100 },
                    { name: '什錦炒意麵', price: 100 },
                    { name: '什錦炒泡麵', price: 100 },
                    { type: 'subtitle', text: '炸物 & 小菜' },
                    { name: '廣島黑輪', price: 30 },
                    { name: '手工黑輪片', price: 30 },
                    { name: '可樂餅', price: 30 },
                    { name: '炸魚排', price: 35 },
                    ...
                    { name: '各式小菜', price: 30 },
                    { name: '皮蛋豆腐', price: 35 },
                    { name: '涼拌香茅牛肉(限量)', price: 60 },
                    { name: '口水雞(限量)', price: 70 },
                    { name: '醉雞(限量)', price: 70 },
                    { type: 'subtitle', text: '飲品' },
                    { name: '玻璃罐飲品', price: 20 },
                    { name: '紅茶', price: 25 },
                    { name: '原萃茶', price: 25 },
                    { name: '紅茶牛奶', price: 40 },
                    { type: 'subtitle', text: '加飯或加麵' },
                    { name: '飯、油麵、泡麵、冬粉', price: '+10' },
                    { name: '雞絲、意麵', price: '+15' },
                    { name: '讚岐烏龍', price: '+25' }
                ]
            },
            { 
                ...
                menu: [
                    ...
                    { name: '玉米濃湯', price: 30 },
                    { name: '海鮮湯', price: 40 },
                    { type: 'subtitle', text: '肉酥' },
                    { name: '起司', price: 30 },
                    { name: '鮪魚', price: 35 },
                    { name: '燻雞', price: 40 },
                    { type: 'subtitle', text: '吐司 / 漢堡' },
                    { name: '卡拉雞腿 (片)', price: 40 },
                    { name: '照燒雞腿 (片)', price: 45 },
                    { name: '起司雞排 (片)', price: 45 },
                    { name: '蚵仔煎', price: 50 },
                    { name: '豬排', price: 40 },
                    { name: '豬排培根', price: 30 },
                    { name: '鮪魚', price: 35 },
                    ...
                    { name: '薯餅', price: 30 },
                    { name: '薯餅', price: 20 },
                    { type: 'subtitle', text: '飲料' },
                    { name: '紅茶', price: '小15 / 大20' },
                    { name: '豆漿', price: '小20 / 大25' },
                    { name: '奶茶', price: '小25 / 大30' },
                    { name: '鮮奶茶', price: '小30 / 大40' },
                    { name: '咖啡牛奶', price: '小30 / 大40' },
                    { name: '草莓牛奶', price: '小30 / 大40' },
                    { name: '冬瓜茶', price: '小20 / 大25' },
                    { name: '檸檬紅茶', price: '小20 / 大25' },
                    { name: '芋頭牛奶', price: '小25 / 大30' },
                    { name: '米漿', price: '小20 / 大25' },
                    { name: '養樂多', price: 25 },
                    { name: '檸檬茶', price: 25 },
                    { name: '研磨芝麻奶茶', price: 35 },
                    { name: '研磨美式咖啡', price: 45 },
                    { name: '研磨拿鐵咖啡', price: 45 },
                    { type: 'subtitle', text: '特製' },
                    { name: '豬肉總匯', price: 55 },
                    { name: '雞腿總匯', price: 60 },
                    { name: '雞腿蛋餅', price: 55 },
                    { name: '豬肉蛋餅', price: 55 },
                    ...
                    { name: '饅頭夾肉', price: 50 },
                    { name: '饅頭夾蛋', price: 25 },
                    { name: '饅頭 (黑/白)', price: 15 }
                ]
            },
            { 
                name: "夏目鐵板燒", 
                displayName: "夏目鐵板燒料理便當", 
                phone: "0925388050", 
                address: "高雄市新興區尚義街119號", 
                info: "營業時間: 11:00 - 18:30\n**注意：店家位於新興區，與鳳山高中距離約 5.6 公里 (開車約 15 分鐘，步行約 1 小時 10 分鐘)，請務必確認外送範圍。**\n備註:\n滿6個便當可外送，外送數量依實際距離調整\n大量訂單請於前一天訂購\n套餐附餐二選一：昆布柴魚清湯 或 古早味紅茶", 
                tags: ["鐵板燒", "便當"], 
                menu: [
                    { type: 'subtitle', text: '便當' },
                    { name: '麻婆豆腐飯', price: 100 },
                    { name: '蔥爆五花豬', price: 110 },
                    { name: '醬燒雞腿肉', price: 110 },
                    { name: '鐵板五花牛', price: 120 },
                    { name: '板煎鱈羊', price: 130 },
                    { name: '好餓配口蝦', price: 140 },
                    { type: 'subtitle', text: '套餐內容: 高麗菜、豆芽菜、日式蒸蛋' },
                    { type: 'subtitle', text: '單點 & 飲品' },
                    { name: '來一碗白飯', price: 10 },
                    { name: '日式水蒸蛋', price: 25 },
                    { name: '古早味紅茶', price: 20 },
                    { name: '濃紅茶鮮奶', price: 35 }
            ...
            { 
                name: "Chai&Tina", 
                phone: "077408206", 
                address: "高雄市鳳山區文衡路491號", 
                info: "電話: (07) 7408206\n營業時間: AM10:30~PM08:00 (每週一公休)\n與鳳山高中距離：約 2.0 公里 (開車約 6 分鐘，步行約 25 分鐘)\n外送服務: 400元以上，外送距離3公里內\n備註: 加奶蓋+10 / 加珍珠+10 / 加椰果+10\n版本日期: 2023.03.14版", 
                tags: ["new", "closed:週一公休", "咖啡", "輕食", "義大利麵", "焗烤", "漢堡", "飲料"], 
                menu: [
                    { type: 'subtitle', text: '義大利麵 Dishes (可選: 青醬/白醬/紅醬/海鮮)' },
                    { name: '義大利麵', price: 120 },
                    { name: '蝦仁義大利麵', price: 130 },
                    { name: '燻雞義大利麵', price: 130 },
                    { name: '德式香腸義大利麵', price: 135 },
                    { name: '培根義大利麵', price: 140 },
                    { name: '蛤蜊義大利麵', price: 150 },
                    { name: '綜合義大利麵', price: 155 },
                    { name: '日式炸豬排義大利麵', price: 150 },
                    { name: '日式炸雞排義大利麵', price: 155 },
                    { name: '日式炸魚排義大利麵', price: 155 },
                    { name: '羅勒燻雞義大利麵', price: 155 },
                    { name: '牛肉義大利麵', price: 160 },
                    { name: '海鮮總匯義大利麵', price: 170 },
                    { type: 'subtitle', text: '焗烤飯類 Risotto (可選: 白醬/紅醬/青醬，焗飯+10元，起司加倍+35元)' },
                    { name: '蔬菜焗烤飯', price: 120 },
                    { name: '鮪魚焗烤飯', price: 125 },
                    { name: '燻雞焗烤飯', price: 130 },
                    { name: '培根焗烤飯', price: 130 },
                    { name: '海鮮焗烤飯', price: 145 },
                    { name: '牛肉焗烤飯', price: 145 },
                    { name: '章魚燒焗烤飯', price: 150 },
                    { name: '綜合焗烤飯', price: 155 },
                    { name: '炸豬排焗烤飯', price: 155 },
                    { name: '炸雞排焗烤飯', price: 160 },
                    { name: '炸魚排焗烤飯', price: 160 },
                    { name: '羅勒燻雞焗烤飯', price: 160 },
                    { name: '海鮮總匯焗烤飯', price: 170 },
                    { type: 'subtitle', text: '披薩 Pizza (皆為6吋)' },
                    { name: '原味', price: 95 },
                    { name: '燻雞起司', price: 110 },
                    { name: '玉米培根', price: 110 },
                    { name: '夏威夷', price: 120 },
                    ...
                    { name: '培根', price: 130 },
                    { name: '小肉豆', price: 140 },
                    { type: 'subtitle', text: '美式總匯輕食系列 (皆附脆薯)' },
                    { name: '鮪魚沙拉', price: 100 },
                    { name: '羅勒雞肉', price: 100 },
                    { name: '日式炸魚排', price: 110 },
                    { name: '培根總匯', price: 110 },
                    { type: 'subtitle', text: '飲料 (不可熱飲)' },
                    { name: '紅茶', price: 25 },
                    { name: '綠茶', price: 25 },
                    { name: '冬瓜茶', price: 25 },
                    { name: '奶茶', price: 30 },
                    ...
                    { name: '奶蓋綠茶', price: 40 },
                    { name: '奶蓋紅茶', price: 40 },
                    { type: 'subtitle', text: '吐司系列 (皆附脆薯)' },
                    { name: '起司蛋吐司', price: 65 },
                    { name: '鮪魚蛋吐司', price: 80 },
                    { name: '燻雞蛋吐司', price: 80 },
                    { name: '豬肉蛋吐司', price: 80 },
                    { type: 'subtitle', text: '漢堡 Burger (可加蛋+10元/起司+10元)' },
                    { name: '原味漢堡', price: '單點80 / 套餐125' },
                    { name: '蔬菜蛋堡', price: '單點80 / 套餐125' },
                    { name: '豬肉漢堡', price: '單點85 / 套餐130' },
                    { name: '雞肉漢堡', price: '單點90 / 套餐135' },
                    { type: 'subtitle', text: '兒童餐 Kids meal' },
                    { name: 'A餐 (麥克雞塊)', price: 65 },
                    { name: 'B餐 (薯條)', price: 65 },
                    { name: 'C餐 (熱狗)', price: 65 },
                    { type: 'subtitle', text: '輕食點心系列' },
                    { name: '熱狗', price: 20 },
                    { name: '雞塊', price: 30 },
                    { name: '薯條', price: 30 },
                    { name: '小肉豆', price: 30 },
            ...
            { 
                name: "大咖", 
                phone: "077406399", 
                address: "高雄市鳳山區中山路138-6號", 
                info: "電話: 07-7406399\n營業時間: 09:00~13:30 / 16:00~20:00\n與鳳山高中距離：約 1.2 公里 (開車約 3 分鐘，步行約 15 分鐘)\n備註: 鳳山區滿300可外送，請提早2小時前來電訂餐。", 
                tags: ["new", "飲料", "蛋包飯", "咖哩"], 
                menu: [
                    { type: 'subtitle', text: '日式咖哩蛋包飯 (內用/外帶)' },
                    { name: '原味', price: 65 },
                    { name: '涮涮豬肉蛋包飯', price: 80 },
                    { name: '海鮮總匯咖哩蛋包飯', price: 80 },
                    { name: '香酥雞排咖哩蛋包飯', price: 80 },
                    { name: '麥克雞塊咖哩蛋包飯', price: 80 },
                    { name: '和風鮮蝦咖哩蛋包飯', price: 80 },
                    { name: '炸花枝排咖哩蛋包飯', price: 80 },
                    { name: '黃金魚排咖哩蛋包飯', price: 80 },
                    { name: '德式香腸咖哩蛋包飯', price: 80 },
                    { name: '日式豬排咖哩蛋包飯', price: 85 },
                    { name: '涮涮牛肉咖哩蛋包飯', price: 95 },
                    { name: '脆皮雞腿咖哩蛋包飯', price: 105 },
                    { type: 'subtitle', text: '焗烤飯' },
                    { name: '原味', price: 95 },
                    { name: '涮涮豬肉咖哩焗烤飯', price: 110 },
                    { name: '海鮮總匯咖哩焗烤飯', price: 110 },
                    { name: '香酥雞排咖哩焗烤飯', price: 110 },
                    { name: '麥克雞塊咖哩焗烤飯', price: 110 },
                    { name: '和風鮮蝦咖哩焗烤飯', price: 110 },
                    { name: '炸花枝排咖哩焗烤飯', price: 110 },
                    { name: '黃金魚排咖哩焗烤飯', price: 110 },
                    { name: '德式香腸咖哩焗烤飯', price: 110 },
                    { name: '日式豬排咖哩焗烤飯', price: 115 },
                    { name: '涮涮牛肉咖哩焗烤飯', price: 125 },
                    { name: '脆皮雞腿咖哩焗烤飯', price: 135 },
                    { type: 'subtitle', text: '咖哩烏龍麵' },
                    { name: '原味咖哩拌麵', price: 85 },
                    { name: '涮涮豬肉咖哩拌麵', price: 100 },
                    { name: '海鮮總匯咖哩拌麵', price: 100 },
                    { name: '香酥雞排咖哩拌麵', price: 100 },
                    { name: '麥克雞塊咖哩拌麵', price: 100 },
                    { name: '和風鮮蝦咖哩拌麵', price: 100 },
                    { name: '炸花枝排咖哩拌麵', price: 100 },
                    { name: '黃金魚排咖哩拌麵', price: 100 },
                    { name: '德式香腸咖哩拌麵', price: 100 },
                    { name: '日式豬排咖哩拌麵', price: 105 },
                    { name: '涮涮牛肉咖哩拌麵', price: 115 },
                    { name: '脆皮雞腿咖哩拌麵', price: 125 },
                    { type: 'subtitle', text: '咖哩烏龍湯麵' },
                    { name: '涮涮豬肉咖哩烏龍湯麵', price: 110 },
                    { name: '海鮮總匯咖哩烏龍湯麵', price: 110 },
                    { name: '香酥雞排咖哩烏龍湯麵', price: 110 },
                    { name: '麥克雞塊咖哩烏龍湯麵', price: 110 },
                    { name: '炸花枝排咖哩烏龍湯麵', price: 110 },
                    { name: '黃金魚排咖哩烏龍湯麵', price: 110 },
                    { name: '德式香腸咖哩烏龍湯麵', price: 110 },
                    { name: '日式豬排咖哩烏龍湯麵', price: 115 },
                    { name: '涮涮牛肉咖哩烏龍湯麵', price: 125 },
                    { name: '脆皮雞腿咖哩烏龍湯麵', price: 135 },
                    { type: 'subtitle', text: '鍋燒系列' },
                    { name: '海鮮鍋燒意麵', price: 75 },
                    { name: '海鮮鍋燒雞絲', price: 75 },
                    { name: '海鮮鍋燒冬粉', price: 75 },
                    { name: '海鮮鍋燒烏龍麵', price: 85 },
                    { name: '泡菜海鮮鍋燒★(附白飯)', price: 100 },
                    { name: '味噌海鮮鍋燒★(附白飯)', price: 100 },
                    { type: 'subtitle', text: '湯品 & 其他' },
                    { name: '白飯', price: 15 },
                    { name: '味噌湯', price: 15 },
                    { name: '蛋花湯', price: 30 },
            ...
            { 
                name: "池上飯盒", 
                displayName: "池上飯盒 鳳山經武店", 
                phone: "077658052", 
                address: "高雄市鳳山區新康街308號", 
                info: "營業時間: AM10:30~PM2:00 / PM4:00~PM8:00\n公休日: 每週日公休\n與鳳山高中距離：約 1.9 公里 (開車約 5 分鐘，步行約 23 分鐘)", 
                tags: ["new", "飯盒", "closed:週日公休"], 
                menu: [
                    { name: '菜飯', price: 65 },
                    { name: '招牌飯', price: 80 },
                    { name: '蝦柳飯', price: 80 },
            ...
            { 
                name: "鍋燒堂", 
                phone: "077752322", 
                address: "高雄市鳳山區凱旋路278號", 
                info: "電話: 07-7752322\n與鳳山高中距離：約 1.0 公里 (開車約 3 分鐘，步行約 12 分鐘)", 
                tags: ["new", "鍋燒", "炒飯", "咖哩"], 
                menu: [
                    { type: 'subtitle', text: '鍋燒類 (加麵/加飯 +15元)' },
                    { name: '經典原味', price: 80 },
                    { name: '香濃沙茶', price: 80 },
                    { name: '招牌牛奶', price: 90 },
                    { name: '南洋咖哩', price: 90 },
                    ...
                    { name: '起司牛奶', price: 105 },
                    { name: '海產粥', price: 90 },
                    { type: 'subtitle', text: '炒飯類 (飯類加飯加10元)' },
                    { name: '肉絲炒飯', price: 80 },
                    { name: '雞肉炒飯', price: 80 },
                    { name: '蔬菜蛋炒飯', price: 80 },
                    { name: '茄汁蛋炒飯', price: 85 },
                    ...
                    { name: '蝦仁蛋炒飯', price: 85 },
                    { name: '泡菜炒飯', price: 90 },
                    { name: '牛肉蛋炒飯', price: 100 },
                    { name: '什錦炒飯', price: 100 },
                    { name: '櫻花蝦炒飯', price: 105 },
                    { type: 'subtitle', text: '咖哩類' },
                    { name: '咖哩飯 (無肉)', price: 70 },
                    { name: '咖哩雞肉飯', price: 80 },
                    { name: '咖哩豬肉飯', price: 80 },
                    { name: '咖哩牛肉飯', price: 95 },
                    { name: '咖哩雞排飯', price: 100 },
                    { name: '咖哩雞腿飯', price: 125 },
                    { type: 'subtitle', text: '蒸飯類 (原味加起司+15元)' },
                    { name: '海苔飯 (無肉)', price: 70 },
                    { name: '海苔雞肉飯', price: 80 },
                    { name: '海苔豬肉飯', price: 80 },
                    { name: '海苔牛肉飯', price: 95 },
                    { name: '海苔雞排飯', price: 100 },
                    { name: '海苔雞腿飯', price: 90 },
                    { type: 'subtitle', text: '單點類' },
                    { name: '白飯', price: 10 },
                    { name: '韓式泡菜', price: 20 },
                    { name: '酥炸黑輪片', price: 40 },
                    { name: '酥炸雞塊', price: 40 },
                    ...
                    { name: '日式炸豬排', price: 55 },
                    { name: '咔啦炸雞排', price: 65 },
                    { type: 'subtitle', text: '紅燒類 (牛肉湯汁)' },
                    { name: '紅燒飯 (無肉)', price: 70 },
                    { name: '紅燒雞肉燴飯', price: 80 },
                    { name: '紅燒豬肉燴飯', price: 80 },
                    { name: '紅燒牛肉燴飯', price: 95 },
                    { name: '紅燒豬排燴飯', price: 100 },
                    { name: '紅燒咔啦雞排', price: 125 },
                    { type: 'subtitle', text: '湯類' },
                    { name: '蛋花湯', price: 30 },
                    { name: '貢丸湯', price: 30 },
                    { name: '蛤蜊湯', price: 40 },
            ...
            { 
                name: "南啵炒飯", 
                phone: "077223704", 
                address: "高雄市苓雅區福德三路342號", 
                info: "電話: 07-7223704\n營業時間: 上午10:00 ~ 晚上9:30\n**注意：店家位於苓雅區，與鳳山高中距離約 3.2 公里 (開車約 8 分鐘，步行約 38 分鐘)，請務必確認外送範圍。**\n備註: 滿300即可外送，請先來電預約，大量訂購請於前一天預訂。", 
                tags: ["new", "炒飯", "麵食"], 
                menu: [
                    { type: 'subtitle', text: '醬炒飯' },
                    { name: '川味肉絲炒飯', price: 100 },
                    { name: '川味雞肉炒飯', price: 90 },
                    { name: '泡菜雞肉炒飯', price: 90 },
                    { name: '泡菜什錦炒意麵', price: 100 },
                    { name: '泡菜豬肉炒飯', price: 90 },
                    { name: '韓式泡菜炒烏龍', price: 95 },
                    { name: '海鮮湯麵', price: 85 },
                    { name: '台式什錦炒飯', price: 80 },
                    { name: '台式什錦炒烏龍', price: 85 },
                    { type: 'subtitle', text: '炒飯 (醬/廣式/蔥/湯)' },
                    { name: 'XO醬蝦仁炒飯', price: 130 },
                    { name: '什錦炒飯', price: 110 },
                    { name: '招牌炒飯', price: 110 },
                    { name: '鹹蛋蝦仁', price: 110 },
                    { name: '玉米炒飯', price: 100 },
                    ...
                    { name: '蛤蠣炒飯', price: 95 },
                    { name: '皮蛋肉絲', price: 95 },
                    { name: '泡菜肉絲炒飯', price: 90 },
                    { name: '培根炒飯', price: 90 },
                    { name: '花枝炒飯', price: 90 },
                    { name: '青椒肉絲炒飯', price: 90 },
                    { name: '青椒牛肉炒飯', price: 100 },
                    { name: '青椒雞肉炒飯', price: 95 },
                    { name: '牛肉炒飯', price: 100 },
                    { name: '雞腿炒飯', price: 95 },
                    { name: '鮭魚炒飯', price: 120 },
                    { name: '鳳梨蝦仁炒飯', price: 110 },
                    { name: '蒜香雞丁炒飯', price: 80 },
                    { name: '蝦仁蛋炒飯', price: 80 },
                    { name: '泡菜蝦仁炒飯', price: 90 },
                    { name: '香腸蛋炒飯', price: 75 },
                    { name: '鹹魚炒飯', price: 80 },
                    { name: '白菜炒飯', price: 70 },
                    { name: '玉米蛋炒飯', price: 70 },
                    ...
                    { name: '肉絲蛋炒飯', price: 70 },
                    { name: '培根蛋炒飯', price: 70 },
                    { name: '香腸肉絲炒飯', price: 75 },
                    { name: '豬排蛋炒飯', price: 75 },
                    { name: '古早味蛋炒飯', price: 65 },
                    { type: 'subtitle', text: '燴飯 / 麵 (泰式/廣式)' },
                    { name: '海鮮燴飯', price: 110 },
                    { name: '滑蛋蝦仁燴飯', price: 100 },
                    { name: '什錦燴飯', price: 95 },
                    { name: '蝦仁燴飯', price: 90 },
                    { name: '肉絲燴飯', price: 90 },
                    { name: '滑蛋肉絲燴飯', price: 90 },
                    { name: '滑蛋牛肉燴飯', price: 100 },
                    { name: '滑蛋雞肉燴飯', price: 95 },
                    { name: '韓式雞肉燴飯', price: 95 },
                    { name: '泰式海鮮燴飯', price: 110 },
                    { name: '泰式雞肉燴飯', price: 100 },
                    { name: '泰式蝦仁燴飯', price: 100 },
                    { name: '港式海鮮燴飯', price: 110 },
                    { name: '港式雞肉燴飯', price: 100 },
                    { name: '港式豬肉燴飯', price: 95 },
                    { name: '泰式牛肉燴飯', price: 100 },
                    { name: '海鮮麵', price: 110 },
                    { name: '什錦麵', price: 100 },
                    { name: '豬肉麵', price: 95 },
                    { name: '雞肉麵', price: 95 },
                    { name: '牛肉麵', price: 100 },
                    { name: '蝦仁麵', price: 95 },
                    { name: '港式海鮮麵', price: 110 },
                    { name: '泰式海鮮麵', price: 110 },
                    { type: 'subtitle', text: '炒麵 / 烏龍麵' },
                    { name: '沙茶豬肉炒麵', price: 90 },
                    { name: '沙茶牛肉炒麵', price: 100 },
                    { name: '沙茶什錦炒意麵', price: 110 },
                    { name: '泡菜豬肉炒烏龍', price: 95 },
                    { name: '川味肉絲炒烏龍', price: 95 },
                    { name: '川味雞肉炒烏龍', price: 90 },
                    { name: '台式什錦炒烏龍', price: 100 },
                    { name: '什錦炒烏龍', price: 90 },
                    { type: 'subtitle', text: '平價快炒' },
                    { name: '打拋豬', price: 120 },
                    { name: '醬爆豬肉', price: 130 },
                    { name: '蔥爆牛肉', price: 130 },
                    { name: '泰式花枝', price: 150 },
                    ...
                    { name: '魚丸湯', price: 40 },
                    { name: '魚羹湯', price: 40 },
                    { type: 'subtitle', text: '季節時蔬' },
                    { name: '清炒水蓮', price: 80 },
                    { name: '清炒菠菜', price: 80 },
                    { name: '清炒龍鬚菜', price: 80 },
                    { name: '櫻花蝦高麗菜', price: 80 },
                    { name: '清炒空心菜', price: 60 },
                    { name: '清炒高麗菜', price: 60 },
                    { name: '清炒小白菜', price: 60 },
                    { type: 'subtitle', text: '鍋燒' },
                    { name: '海產羹', price: 120 },
                    { name: '韓式泡菜海鮮麵', price: 130 },
                    { name: '韓式泡菜肉絲麵', price: 95 },
                    { name: '招牌烏龍麵', price: 90 },
                    { name: '鍋燒泡飯', price: 85 },
                    { name: '蛤蠣麵', price: 85 },
                    { name: '豬肉湯', price: 80 },
                    { type: 'subtitle', text: '滷味' },
                    { name: '白飯', price: 10 },
                    { name: '荷包蛋', price: 20 }
                ]
            },
            ...
            { 
                name: "梅家村排骨飯", 
                displayName: "梅家村排骨飯 鳳山店", 
                phone: "077990002", 
                address: "高雄市鳳山區青年路二段2號", 
                info: "電話: 07-7990002 / 07-7990609\n營業時間: 週六下午二點後店休 / 週日店休\n與鳳山高中距離: 約 1.6 公里 (開車約 5 分鐘，步行約 20 分鐘)\n歡迎團體訂購", 
                tags: ["new", "飯", "排骨飯", "closed:週六下午/週日公休"], 
                menu: [
                    { name: '排骨飯', price: 95 },
                    { name: '雞腿飯', price: 115 },
                    { name: '魯肉飯', price: 95 },
            ...
            { 
                name: "麥圓多", 
                displayName: "麥圓多 日式咖哩蛋包飯", 
                phone: "077218689", 
                address: "鳳山區南江街130號", 
                info: "營業時間:\n上午 09:00 ~ 下午 14:00\n下午 16:00 ~ 晚上 20:30\n與鳳山高中距離：約 1.7 公里 (開車約 4-5 分鐘，步行約 20 分鐘)\n外送服務: 滿300元即可外送 (視距離而定)，請於一小時前來電預訂。", 
                tags: ["new", "早午餐", "咖哩", "蛋包飯"], 
                menu: [
                    { type: 'subtitle', text: '蛋包飯 (皆可焗烤+30元)' },
                    { name: '原味咖哩蛋包飯 (無肉)', price: 65 },
                    { name: '鱈魚咖哩蛋包飯', price: 80 },
                    { name: '麥克雞塊咖哩蛋包飯', price: 80 },
                    { name: '黃金鮮蝦咖哩蛋包飯', price: 80 },
                    { name: '海鮮總匯咖哩蛋包飯', price: 80 },
                    { name: '川燙豬肉片咖哩蛋包飯', price: 80 },
                    { name: '雞排咖哩蛋包飯', price: 85 },
                    { name: '雞肉丁咖哩蛋包飯', price: 90 },
                    { name: '卡滋豬排咖哩蛋包飯', price: 90 },
                    { name: '牛肉咖哩蛋包飯', price: 95 },
                    { name: '脆皮雞腿咖哩蛋包飯', price: 105 },
                    { name: '無骨雞腿咖哩蛋包飯', price: 105 },
                    { type: 'subtitle', text: '咖哩拌麵 (皆可焗烤+30元)' },
                    { name: '原味咖哩拌麵 (無肉)', price: 85 },
                    { name: '鱈魚咖哩拌麵', price: 100 },
                    { name: '麥克雞塊咖哩拌麵', price: 100 },
                    { name: '黃金鮮蝦咖哩拌麵', price: 100 },
                    { name: '海鮮總匯咖哩拌麵', price: 100 },
                    { name: '川燙豬肉片咖哩拌麵', price: 100 },
                    { name: '雞排咖哩拌麵', price: 105 },
                    { name: '雞肉丁咖哩拌麵', price: 110 },
                    { name: '卡滋豬排咖哩拌麵', price: 110 },
                    { name: '牛肉咖哩拌麵', price: 115 },
                    { name: '脆皮雞腿咖哩拌麵', price: 125 },
                    { name: '無骨雞腿咖哩拌麵', price: 125 },
                    { type: 'subtitle', text: '咖哩雞絲麵' },
                    { name: '原味', price: 75 },
                    { name: '雞肉', price: 90 },
                    { name: '雞排', price: 95 },
                    { name: '雞肉丁', price: 100 },
                    ...
                    { name: '原味', price: 85 },
                    { name: '海鮮總匯咖哩烏龍湯麵', price: 110 },
                    { name: '豬肉咖哩烏龍湯麵', price: 110 },
                    { name: '牛肉咖哩烏龍湯麵', price: 110 },
                    { type: 'subtitle', text: '焗烤飯' },
                    { name: '原味咖哩焗烤飯', price: 95 },
                    { name: '鱈魚咖哩焗烤飯', price: 110 },
                    { name: '麥克雞塊咖哩焗烤飯', price: 110 },
                    { name: '黃金鮮蝦咖哩焗烤飯', price: 110 },
                    { name: '海鮮總匯咖哩焗烤飯', price: 110 },
                    { name: '川燙豬肉片咖哩焗烤飯', price: 110 },
                    { name: '雞排咖哩焗烤飯', price: 115 },
                    { name: '雞肉丁咖哩焗烤飯', price: 120 },
                    { name: '卡滋豬排咖哩焗烤飯', price: 120 },
                    { name: '牛肉咖哩焗烤飯', price: 125 },
                    { name: '脆皮雞腿咖哩焗烤飯', price: 135 },
                    { name: '無骨雞腿咖哩焗烤飯', price: 135 },
                    { type: 'subtitle', text: '湯類' },
                    { name: '味噌', price: 15 },
                    { name: '玉米濃湯', price: 25 },
                    { name: '海鮮湯', price: 55 },
    ...
    function renderVendors(filter = '') { /* ...無變動... */ }
    function populateAndShowModal(vendorName) { /* ...無變動... */ }
    function openModal() { /* ...無變動... */ }
    function closeModal() { /* ...無變動... */ }
    function initDynamicBackground() { /* ...無變動... */ }
    // (無變動部分省略)
    function renderVendors(filter = '') {
        if (!vendorGrid) return; vendorGrid.innerHTML = ''; const lowerCaseFilter = filter.toLowerCase();
    ...
    function populateAndShowModal(vendorName) {
        const vendor = vendorData.find(v => v.name === vendorName); if (!vendor || !modal) return;
        modal.querySelector('#modal-vendor-name').textContent = vendor.displayName || vendor.name;
        const phoneLink = modal.querySelector('#modal-vendor-phone'); if (vendor.phone && vendor.phone !== "無") { phoneLink.href = `tel:${vendor.phone}`; phoneLink.style.display = 'inline-flex'; modal.querySelector('#modal-vendor-phone-text').textContent = vendor.phone; } else { phoneLink.style.display = 'none'; }
        ...
        const tagsContainer = modal.querySelector('#modal-vendor-tags'); tagsContainer.innerHTML = ''; if (vendor.tags) { vendor.tags.forEach(tag => { let className = 'vendor-tag'; let tagText = tag; if (tag.startsWith('new')) { className += ' new'; tagText = '新店家'; } else if (tag.startsWith('closed:')) { className += ' closed'; tagText = tag.replace('closed:', ''); } tagsContainer.innerHTML += `<span class="${className}">${tagText}</span>`; }); }
    ...
    function initDynamicBackground() {
        ...
        const bgImages = menuBackgroundContainer.querySelectorAll('.bg-image'); let currentIndex = 0;
