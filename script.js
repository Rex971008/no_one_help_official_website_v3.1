...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchPage(targetId) {
        ...
        // 未來月曆點餐也可能需要登入，可以加上類似的檢查
        // if (targetId === 'page-calendar' && !Auth.isLoggedIn()) {
        //     targetId = 'page-login';
        // }
    ...
    if (registerBtn) registerBtn.addEventListener('click', async () => { /* ... 內容不變 ... */ });
    if (loginBtn) loginBtn.addEventListener('click', async () => { /* ... 內容不變 ... */ });
    if (logoutBtn) logoutBtn.addEventListener('click', () => { /* ... 內容不變 ... */ });
    ...
    // (將 registerBtn 和 loginBtn 的詳細邏輯放回)
    ...
    // ================== 功能性函數 ==================
    // --- 聊天功能 ---
    function startChatSession() { /* ... 內容不變 ... */ }
    function stopChatSession() { /* ... 內容不變 ... */ }
    async function fetchChatMessages() { /* ... 內容不變 ... */ }
    async function sendChatMessage() { /* ... 內容不變 ... */ }
    ...
    function renderCalendar() {
        ...
        for (let i = 0; i < firstDayOfMonth; i++) {
            ...
            emptyDay.className = 'calendar-day not-current-month';
    ...
    function setupOfficialWebsiteJS() {
        // ... (這裡的內容和上一版「黃金最終版」完全相同)
    }
    function renderVendors(filter = '') { /* ... 內容不變 ... */ }
    function populateAndShowModal(vendorName) { /* ... 內容不變 ... */ }
    function openModal() { /* ... 內容不變 ... */ }
    function closeModal() { /* ... 內容不變 ... */ }
    function initDynamicBackground() { /* ... 內容不變 ... */ }
    ...
    function renderVendors(filter = '') {
        ...
        const filteredData = vendorData.filter(vendor => {
            if (!vendor.tags) vendor.tags = []; // 防呆，避免 tags 是 undefined
